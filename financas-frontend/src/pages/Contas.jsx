...
export default function Contas() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (form.id) {
      api.put(`/parametros/contas/${form.id}`, form)
        .then(res => {
          setContas(contas.map(c => c.id === form.id ? res.data : c));
          setShowModal(false); setForm(initialForm()); setEditando(false);
        })
        .catch(err => console.error("Erro ao editar conta:", err));
    } else {
      api.post("/parametros/contas", form)
        .then(res => {
  ...
  const handleDelete = (id) => {
    if (window.confirm("Deseja excluir esta conta?")) {
      api.delete(`/parametros/contas/${id}`)
        .then(() => setContas(contas.filter(c => c.id !== id)))
  ...
  return (
    <div className="p-8 bg-gray-900 min-h-screen text-gray-100 relative">
      <h1 className="text-2xl font-bold mb-6">💳 Contas / Cartões</h1>
      ...
      <button
        ...
        className="fixed bottom-20 right-6 bg-green-600 hover:bg-green-500 text-white rounded-full w-14 h-14 text-3xl shadow-lg flex items-center justify-center"
      >
        +
      </button>
