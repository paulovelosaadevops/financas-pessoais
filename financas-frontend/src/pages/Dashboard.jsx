    ...
    <div className="min-h-screen bg-gray-900 text-gray-100 p-8">
      ...
      <header className="mb-10 rounded-2xl bg-gradient-to-r from-gray-900 via-gray-950 to-black p-6 shadow-lg border border-gray-800">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 className="text-[4.5rem] font-assinatura text-amber-400 leading-none drop-shadow-[0_2px_4px_rgba(0,0,0,0.6)]">
              Família Bertão
            </h1>
          ...
          <div className="bg-gray-800 p-6 rounded-xl shadow-xl w-96">
            ...
            <div className="mt-5 flex gap-3">
              <button onClick={exportarRelatorio}
                className="flex-1 bg-green-600 hover:bg-green-700 text-white py-2 rounded">Gerar Excel</button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
        <div className="bg-gray-800 p-6 rounded-xl shadow-lg min-h-[350px]">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
              <Pie data={resumo.fixasCategorias || []} cx="50%" cy="50%" outerRadius={100} dataKey="total" nameKey="nome" label={renderLabel}>
                {resumo.fixasCategorias?.map((_, i) => (<Cell key={i} fill={COLORS_FIXAS[i % COLORS_FIXAS.length]} />))}
              </Pie>
              <Tooltip formatter={(v) => formatCurrency(v)} />
            </PieChart>
          </ResponsiveContainer>
        ...
        <div className="bg-gray-800 p-6 rounded-xl shadow-lg min-h-[350px]">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
              <Pie data={resumo.fixasResponsaveis || []} cx="50%" cy="50%" outerRadius={100} dataKey="total" nameKey="nome" label={renderLabel}>
                {resumo.fixasResponsaveis?.map((_, i) => (<Cell key={i} fill={COLORS_FIXAS[i % COLORS_FIXAS.length]} />))}
              </Pie>
              <Tooltip formatter={(v) => formatCurrency(v)} />
            </PieChart>
          </ResponsiveContainer>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
        <div className="bg-gray-800 p-6 rounded-xl shadow-lg min-h-[350px]">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
              <Pie data={resumo.receitasCategorias || []} cx="50%" cy="50%" outerRadius={100} dataKey="total" nameKey="nome" label={renderLabel}>
                {resumo.receitasCategorias?.map((_, i) => (<Cell key={i} fill={COLORS_RECEITAS[i % COLORS_RECEITAS.length]} />))}
              </Pie>
              <Tooltip formatter={(v) => formatCurrency(v)} />
            </PieChart>
          </ResponsiveContainer>
        ...
        <div className="bg-gray-800 p-6 rounded-xl shadow-lg min-h-[350px]">
          ...
          <ResponsiveContainer width="100%" height={280}>
            <PieChart>
              <Pie data={resumo.receitasResponsaveis || []} cx="50%" cy="50%" outerRadius={100} dataKey="total" nameKey="nome" label={renderLabel}>
                {resumo.receitasResponsaveis?.map((_, i) => (<Cell key={i} fill={COLORS_RECEITAS[i % COLORS_RECEITAS.length]} />))}
              </Pie>
              <Tooltip formatter={(v) => formatCurrency(v)} />
            </PieChart>
          </ResponsiveContainer>
    ...
    <table className="w-full text-left border-collapse">
      ...
      <tbody>
        {resumo.ultimosLancamentos?.length > 0 ? (
          ...
          <tr>
            <td colSpan="3" className="p-4 text-center text-gray-400">Nenhum lançamento encontrado.</td>
          </tr>
        )}
      </tbody>
    </table>
